...
HOPE. The share shifts reflect real reallocation---approximately 10 additional
bottom-quintile students per cohort enter, 29 top-quintile students per
...
Tennessee privates, consistent with out-of-state enrollment as the
likely exit margin. Third, two-year and four-year institutions adjust through
...
We estimate treatment effects using two-way fixed effects (TWFE) as the primary estimator.
Because treatment timing is sharp (all Tennessee public colleges
treated simultaneously), heterogeneity-robust estimators designed for
staggered adoption reduce to TWFE in our setting
\parencite{roth2026interpreting}; we report
\textcite{sun2021estimating} and \textcite{dechaisemartin2020two} for
robustness. The preferred
specification controls for pre-treatment county average income (Bureau of Economic Analysis),
which absorbs cross-county differences in baseline economic conditions. Results are stable across
three treatment timing definitions and two alternative control group
specifications. Under the relative-magnitude restriction of
\textcite{rambachan2023more}, the estimated effect survives
post-treatment violations as large as the maximum pre-treatment
violation ($\bar{M} \leq 1$). Under the smoothness restriction, both
bottom- and top-quintile share are robust at $M \leq 0.005$. The
...
We make four contributions. First, we provide the first
...
confounded by enrollment contraction). Fourth, we decompose the
scholarship's compositional effect into a permanent price effect
(progressive sorting) and a cyclical income-sensitivity effect
(responsiveness to economic downturns). Merging BEA county income data
with the Chetty panel, we show that tuition-scaling programs (Florida,
West Virginia) compensate the income-sensitivity effect---composition
is stable during downturns---while fixed-dollar programs (Tennessee,
Georgia) do not. This decomposition produces a second five-for-five
monotonic quintile gradient, estimated on entirely different variation.
A third gradient emerges at four-year colleges under fixed-dollar
programs, where marginal students downshift to two-year institutions
during income shocks. Three independent monotonic gradients---from
program adoption, shock buffering, and shock-induced tier
reallocation---converge on the same price-theoretic mechanism and
identify award structure as the key design parameter determining
whether progressive sorting is sustained.

The paper proceeds as follows. Section~\ref{sec:background} describes
the Tennessee HOPE Scholarship, the Georgia and Florida programs that
serve as comparison cases, and the prior literature.
Section~\ref{sec:theory} presents the three-destination sorting model,
derives testable predictions, and introduces the income-shock
decomposition. Section~\ref{sec:data} describes the Chetty panel,
BEA income data, and merit program classification.
Section~\ref{sec:empirical_strategy} presents the adoption DiD and
shock-interaction designs. Section~\ref{sec:results} presents results
for both designs, including the quintile gradient, shock-insurance
gradient, tier reallocation, and award-structure findings.
Section~\ref{sec:robustness} reports robustness checks.
Section~\ref{sec:discussion} discusses cross-state evidence, the
decomposition interpretation, and policy implications.
