...
estimates under the preferred specification. Following HOPE
implementation, the share of bottom-quintile students at Tennessee
public colleges increased by 1.26 percentage points (SE = 0.0023,
$p < 0.001$) and mean parental income rank decreased by 0.81
percentage points (SE = 0.0018, $p < 0.001$). Both effects are
precisely estimated and economically meaningful: the bottom-quintile
share increase represents a 9.1\% gain relative to the pre-treatment
mean of 13.8\%. In levels, the effect corresponds to approximately
10.3 additional bottom-quintile students per cohort at each Tennessee
public college and 28.6 fewer top-quintile students per cohort
...
estimates---1.42 pp for bottom-quintile share and $-1.48$ pp for
mean parental income rank (Table~\ref{tab:twfe_did})---consistent with
the known upward bias of TWFE in the presence of dynamic treatment
effects. The qualitative conclusions are identical across estimators.
...
that the number of bottom-quintile students per cohort increases by 10.3
($p = 0.063$), the number of top-quintile students per cohort decreases
by 28.6 ($p = 0.003$), and total enrollment is unchanged ($+8.1$,
$p = 0.78$). The public sector absorbs more lower-income students while
...
colleges is essentially unchanged after HOPE ($+8.1$ students per
cohort, $p = 0.78$). This rules out the denominator-artifact
...
Table~\ref{tab:spillover} reports estimates comparing Tennessee private
colleges to private colleges in control states. The effect on
...
The within-Tennessee DiD estimate for bottom-quintile share is 1.56
...
border colleges. This is consistent with E1: where students have
...
= -0.178$ vs.\ Q1's $-0.149$). This is consistent with E2: Q2
...
P1: Q1 share rises & C\&S ATT & \ref{sec:results_main} & $+1.3$ pp ($p < 0.001$) \\
...
E1: Spatial gradient & Border vs.\ interior & \ref{sec:spatial} & Border 38\% larger \\
E2: Q2 most elastic & Quintile decomp. & \ref{sec:results_tier_reallocation} & Q2 $> $ Q1 at 4-yr \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Notes:} P1--P5, P7--P10 are derived from the theoretical
  ...
  E1--E2 are empirical implications consistent with the model but not
  directly derived from it (the model contains no geographic component
  and does not predict quintile-specific elasticities).
