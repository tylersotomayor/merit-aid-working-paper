...
\begin{figure}[H]
  ...
  \begin{minipage}{0.9\textwidth}
    \footnotesize
    \textit{Notes:} Event-study estimates from three heterogeneity-robust
    estimators (\textcite{callaway2021difference}, \textcite{sun2021estimating},
    and TWFE), estimated separately for each income quintile share.
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.9\textwidth}
    ...
    \textcite{callaway2021difference} estimator. Data:
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.9\textwidth}
    \footnotesize
    \textit{Notes:} \textcite{callaway2021difference} event-study
    estimates for count outcomes (quintile share $\times$ cohort
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.9\textwidth}
    \footnotesize
    \textit{Notes:} \textcite{callaway2021difference} event-study
    estimates comparing Tennessee private colleges (treated) to private
...
% Note: Combined R&R figures (fig_rr_combined_par_q1, fig_rr_combined_par_rank)
% are now inline in Section 7 (Robustness). Only the breakdown plot remains here.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{rr_breakdown_varying.pdf}
  \caption{Sensitivity Analysis: Breakdown Values Across Post-Treatment Event Times}
  \label{fig:rr_breakdown}
  \vspace{1mm}
  \begin{minipage}{0.9\textwidth}
    \footnotesize
    \textit{Notes:} Breakdown values of $\bar{M}$ at each post-treatment
    event time under both the relative magnitude ($\Delta_\text{RM}$)
    and smoothness ($\Delta_\text{SD}$) restrictions of
    \textcite{rambachan2023more}. Higher breakdown values indicate
    greater robustness to violations of parallel trends. Data:
    \textcite{chetty2020mobility}.
  \end{minipage}
\end{figure}
